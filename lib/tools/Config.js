...
var Config = module.exports = {
  ...
  // Cache for compiled regex patterns to prevent ReDoS
  _regexCache: {},
  // Maximum input length for regex validation to prevent ReDoS
  _maxRegexInputLength: 10000,
  /**
   * Schema definition.
   * @returns {exports|*}
   */
...
Config._valid = function(key, value, sch){
  ...
  if (type == '[object String]' && sch.regex) {
    ...
    if (value && value.length > Config._maxRegexInputLength) {
      if (this._error(true, 'regex', key, 'Input exceeds maximum length of ' + Config._maxRegexInputLength + ' characters')) {
        return null;
      }
    }
    // Cache compiled regex to prevent ReDoS and improve performance
    if (!Config._regexCache[sch.regex]) {
      try {
        Config._regexCache[sch.regex] = new RegExp(sch.regex);
      } catch (e) {
        if (this._error(true, 'regex', key, 'Invalid regex pattern: ' + e.message)) {
          return null;
        }
      }
    }
    if (this._error(!Config._regexCache[sch.regex].test(value),
